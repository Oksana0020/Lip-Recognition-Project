...
from training.dataset import WordDataset
from training.device import resolve_device
from training.train_utils import (
    build_sampler, train_one_epoch, eval_one_epoch, save_checkpoint)

PROJECT_ROOT = Path(__file__).parent.parent
DATA_ROOT = PROJECT_ROOT / "data" / "processed" / "words_by_label"
BBOX_JSON = PROJECT_ROOT / "data" / "processed" / "words_lip_bboxes.json"
CHECKPOINT_DIR = PROJECT_ROOT / "training" / "checkpoints_words"
TENSORBOARD_DIR = PROJECT_ROOT / "training" / "runs_words"
NUM_FRAMES = 8
HEIGHT = 64
WIDTH = 64
BATCH_SIZE = 64
...
def main() -> None:
    ...
    full = WordDataset(
        ...
        lip_bbox_lookup=lip_bbox_lookup,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            no_improve = 0
            save_checkpoint(
                CHECKPOINT_DIR / "best_model_words.pth",
                model, optimiser, epoch, val_acc,
                label_map=word_to_index,
                extra={"index_to_word": index_to_word}
            )
        else:
            no_improve += 1
